...
export default class lichsuthuhoi extends Component {
  constructor(props) {
    super(props);
    this.state = {
        loading: true,
...
getdata = async () => {
    return await fetch(url + "taophieuthu")
...
getdatachitietphieu = async (id) => {
    return await fetch(url + `taophieuthu/${id}`)
        .then((response) => response.json())
        .then((responseJson) => {
            this.setState({
               chitietphieuthuhoi: responseJson,
        ...
        <View style={styles.MainContainer}>
            <View style={styles.header_footer_style}>
                <TextInput
                    ...
                    placeholder="Nhập ID Phiếu Thu Hồi..."
                />
            </View>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        ...
                        <View style={styles.viewinflatlist}>
                            <View >
                                <Ionicons name="ios-git-compare" size={25} color="#4F5704" style={{ paddingLeft: 10 }} />
                            </View>
                            <View style={styles.textinflatlist}>
                                <Text style={styles.text}> ID Phiếu Thu Hồi: {item.id_th}</Text>
                                <Text style={styles.text}> Mô Tả: {item.ma_phieu}</Text>
                                <Text style={styles.text}> Tên Kho Thu Hồi:  {item.ten_kho_con}</Text>
                                <Text style={styles.text}> Ngày Thu Hồi:  {item.ngay_thu_hoi}</Text>
                            </View>
            ...
            > 
                <View style={{
                    ...
                    {
                        this.state.chitietphieuthuhoi.map((item,key) => {
                            return (
                                <View 
                                key ={key}
                                style={{
                                    ...
                                    <Text>  Mô Tả :{item.ma_phieu}  </Text>
                                    <Text>  Tên Kho Con: {item.ten_kho_con}  </Text>
                                    <Text>  Tên Người Thu Hồi: {item.ten_nguoi_giao}  </Text>
                                    <Text>  Ngày Thu Hồi : {item.ngay_thu_hoi}  </Text>
